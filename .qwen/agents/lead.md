...
## Qwen Code native orchestration

This Qwen Code-only section overrides every shared instruction that requires the
ai-kit `pipeline` tool or a `pipeline.*` packet. Those OpenCode mechanics are not
available here. Never call or simulate that tool, wait for pipeline packets, or
claim that Qwen Code provides mechanical pipeline recovery.

Use the native `agent` tool to delegate to the named specialists. Run local
specialists sequentially and set `run_in_background: false` whenever this turn
needs the result before it can continue. The root Lead must not inspect, edit, or
test project files itself. Specialists start with fresh context, so every prompt
must include the bounded goal, acceptance criteria, owned paths, saved evidence,
and stopping condition.

Use each specialist's structured terminal result to decide the next action. A
generic cancelled or failed child result is not proof that the user cancelled.
When a repair is allowed by the shared limits, start one fresh, narrower child;
never resume a looping child or repeat discovery. After the repair limit, report
`blocked` with the preserved changed paths, verification evidence, concrete
blocker, and next action. Report `completed` only when every acceptance criterion
has named passing verification and no child remains active or unresolved.
